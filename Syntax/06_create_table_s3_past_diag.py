#------------------------------------------------------------------------------#
# Create Table S3 (Past Diagnoses)
# Author: Jianhui Sun
#------------------------------------------------------------------------------#
...
past_diag_neural = {'GAD': [], 'OCD': [], 'Panic': [], 'Agor': [], 'PSTD': [], 'SAD': [], 'Phobias': [], 
...
for index, row in NEUTRAL.iterrows():
    ...
    for key in past_diag_neural:
        if key in diag:
            past_diag_neural[key].append(ID)
            

past_diag_fifty_fifty_random = {'GAD': [], 'OCD': [], 'Panic': [], 'Agor': [], 'PSTD': [], 'SAD': [], 'Phobias': [], 
            'dem': [], 'sbst': [], 'schiz': [], 'depression': [], 'bipolar': [], 'eating': [], 'personality': [], 
            '555': [], 'noDiagnosis': [], 'Other': []}
for index, row in FIFTY_FIFTY_RANDOM.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all disorders
    for key in past_diag_fifty_fifty_random:
        if key in diag:
            past_diag_fifty_fifty_random[key].append(ID)
            
            
past_diag_positive_negation = {'GAD': [], 'OCD': [], 'Panic': [], 'Agor': [], 'PSTD': [], 'SAD': [], 'Phobias': [], 
            'dem': [], 'sbst': [], 'schiz': [], 'depression': [], 'bipolar': [], 'eating': [], 'personality': [], 
            '555': [], 'noDiagnosis': [], 'Other': []}
for index, row in POSITIVE_NEGATION.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all disorders
    for key in past_diag_positive_negation:
        if key in diag:
            past_diag_positive_negation[key].append(ID)
            

past_diag_positive = {'GAD': [], 'OCD': [], 'Panic': [], 'Agor': [], 'PSTD': [], 'SAD': [], 'Phobias': [], 
            'dem': [], 'sbst': [], 'schiz': [], 'depression': [], 'bipolar': [], 'eating': [], 'personality': [], 
            '555': [], 'noDiagnosis': [], 'Other': []}
for index, row in POSITIVE.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all disorders
    for key in past_diag_positive:
        if key in diag:
            past_diag_positive[key].append(ID)


past_diag_fifty_fifty_blocked = {'GAD': [], 'OCD': [], 'Panic': [], 'Agor': [], 'PSTD': [], 'SAD': [], 'Phobias': [], 
            'dem': [], 'sbst': [], 'schiz': [], 'depression': [], 'bipolar': [], 'eating': [], 'personality': [], 
            '555': [], 'noDiagnosis': [], 'Other': []}
for index, row in FIFTY_FIFTY_BLOCKED.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all disorders
    for key in past_diag_fifty_fifty_blocked:
        if key in diag:
            past_diag_fifty_fifty_blocked[key].append(ID)
            

       
freq_table_past_diag = { 
...
print('NEUTRAL')
for key, value in freq_table_past_diag['NEUTRAL'].items():
    value = len(past_diag_neural[key]) / group_size['NEUTRAL']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))
    value = str("{:.1f}".format(value))

    freq_table_past_diag['NEUTRAL'][key] = value
...
for key, value in freq_table_past_diag['FIFTY_FIFTY_RANDOM'].items():
    value = len(past_diag_fifty_fifty_random[key]) / group_size['FIFTY_FIFTY_RANDOM']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_past_diag['FIFTY_FIFTY_RANDOM'][key] = value
...
for key, value in freq_table_past_diag['POSITIVE_NEGATION'].items():
    value = len(past_diag_positive_negation[key]) / group_size['POSITIVE_NEGATION']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_past_diag['POSITIVE_NEGATION'][key] = value
...
print('POSITIVE')
for key, value in freq_table_past_diag['POSITIVE'].items():
    value = len(past_diag_positive[key]) / group_size['POSITIVE']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_past_diag['POSITIVE'][key] = value
...
for key, value in freq_table_past_diag['FIFTY_FIFTY_BLOCKED'].items():
    value = len(past_diag_fifty_fifty_blocked[key]) / group_size['FIFTY_FIFTY_BLOCKED']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_past_diag['FIFTY_FIFTY_BLOCKED'][key] = value
...
for outerKey, innerDict in freq_table_past_diag.items():
    for innerKey, values in innerDict.items():
        reformed_dict[innerKey].append(values)
  
past_diag_table = pd.DataFrame.from_dict(reformed_dict, orient='index', columns=['POSITIVE_NEGATION', 'POSITIVE', 'FIFTY_FIFTY_BLOCKED', 'FIFTY_FIFTY_RANDOM', 'NEUTRAL'])
past_diag_table.to_excel('past_diag_table.xlsx')
