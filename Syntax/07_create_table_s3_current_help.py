...
# noDiagnosis = I am not seeking help
# Other = Other (Please specify)
###
    
# process the current help: remove all special characters (including meaningless 'u') and split by ','
for single_patient in patient_record:
    
    intermediate = single_patient['cur_help']
    intermediate = intermediate.translate({ord(i): None for i in "u[]''"})
    intermediate = intermediate.split(", ")
    single_patient['cur_help'] = intermediate
...
cur_help_neural = {'psychiatrist': [], 'psychologist': [], 'school_conselor': [], 'LMHC': [], 'general_practitioner': [],
            'teacher': [], 'family': [], 'friend': [], 'religios_leader': [], 'coach': [], 'book': [], 'medicine': [], 
            'online': [], 'app': [], 'spport_grop': [], '555': [], 'noDiagnosis': [], 'Other': []}
...
for index, row in NEUTRAL.iterrows():
    ...
    for key in cur_help_neural:
        if key in diag:
            cur_help_neural[key].append(ID)
            

cur_help_fifty_fifty_random = {'psychiatrist': [], 'psychologist': [], 'school_conselor': [], 'LMHC': [], 'general_practitioner': [],
            'teacher': [], 'family': [], 'friend': [], 'religios_leader': [], 'coach': [], 'book': [], 'medicine': [], 
            'online': [], 'app': [], 'spport_grop': [], '555': [], 'noDiagnosis': [], 'Other': []}
...
for index, row in FIFTY_FIFTY_RANDOM.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all helps
    for key in cur_help_fifty_fifty_random:
        if key in diag:
            cur_help_fifty_fifty_random[key].append(ID)
            
            
cur_help_positive_negation = {'psychiatrist': [], 'psychologist': [], 'school_conselor': [], 'LMHC': [], 'general_practitioner': [],
            'teacher': [], 'family': [], 'friend': [], 'religios_leader': [], 'coach': [], 'book': [], 'medicine': [], 
            'online': [], 'app': [], 'spport_grop': [], '555': [], 'noDiagnosis': [], 'Other': []}
...
for index, row in POSITIVE_NEGATION.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all helps
    for key in cur_help_positive_negation:
        if key in diag:
            cur_help_positive_negation[key].append(ID)
            

cur_help_positive = {'psychiatrist': [], 'psychologist': [], 'school_conselor': [], 'LMHC': [], 'general_practitioner': [],
            'teacher': [], 'family': [], 'friend': [], 'religios_leader': [], 'coach': [], 'book': [], 'medicine': [], 
            'online': [], 'app': [], 'spport_grop': [], '555': [], 'noDiagnosis': [], 'Other': []}
...
for index, row in POSITIVE.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all helps
    for key in cur_help_positive:
        if key in diag:
            cur_help_positive[key].append(ID)
            

cur_help_fifty_fifty_blocked = {'psychiatrist': [], 'psychologist': [], 'school_conselor': [], 'LMHC': [], 'general_practitioner': [],
            'teacher': [], 'family': [], 'friend': [], 'religios_leader': [], 'coach': [], 'book': [], 'medicine': [], 
            'online': [], 'app': [], 'spport_grop': [], '555': [], 'noDiagnosis': [], 'Other': []}
...
for index, row in FIFTY_FIFTY_BLOCKED.iterrows():
    ...
    ID = int(row['ID'])
    ### iterate over all helps
    for key in cur_help_fifty_fifty_blocked:
        if key in diag:
            cur_help_fifty_fifty_blocked[key].append(ID)
...
freq_table_cur_help = {
                       ...
                       'POSITIVE_NEGATION': {'psychiatrist': None, 'psychologist': None, 'school_conselor': None, 'LMHC': None, 'general_practitioner': None, 'teacher': None, 'family': None, 
                                             ...
                                             'online': None, 'app': None, 'spport_grop': None, 'Other': None, 'noDiagnosis': None, '555': None},
...
print('NEUTRAL')
for key, value in freq_table_cur_help['NEUTRAL'].items():
    value = len(cur_help_neural[key]) / group_size['NEUTRAL']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_cur_help['NEUTRAL'][key] = value
...
for key, value in freq_table_cur_help['FIFTY_FIFTY_RANDOM'].items():
    value = len(cur_help_fifty_fifty_random[key]) / group_size['FIFTY_FIFTY_RANDOM']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_cur_help['FIFTY_FIFTY_RANDOM'][key] = value
...
for key, value in freq_table_cur_help['POSITIVE_NEGATION'].items():
    value = len(cur_help_positive_negation[key]) / group_size['POSITIVE_NEGATION']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))
    value = str("{:.1f}".format(value))

    freq_table_cur_help['POSITIVE_NEGATION'][key] = value
...
print('POSITIVE')
for key, value in freq_table_cur_help['POSITIVE'].items():
    value = len(cur_help_positive[key]) / group_size['POSITIVE']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))
    value = str("{:.1f}".format(value))

    freq_table_cur_help['POSITIVE'][key] = value
...
for key, value in freq_table_cur_help['FIFTY_FIFTY_BLOCKED'].items():
    value = len(cur_help_fifty_fifty_blocked[key]) / group_size['FIFTY_FIFTY_BLOCKED']
    value = str("{:.1%}".format(value))
    value = float(value.translate({ord(i): None for i in "%"}))    
    value = str("{:.1f}".format(value))

    freq_table_cur_help['FIFTY_FIFTY_BLOCKED'][key] = value
...
reformed_dict = {'psychiatrist': [], 'psychologist': [], 'school_conselor': [], 'LMHC': [], 'general_practitioner': [], 'teacher': [], 'family': [], 
                 ...
                 'online': [], 'app': [], 'spport_grop': [], 'Other': [], 'noDiagnosis': [], '555': []}

for outerKey, innerDict in freq_table_cur_help.items():
    for innerKey, values in innerDict.items():
        reformed_dict[innerKey].append(values)
  
cur_help_table = pd.DataFrame.from_dict(reformed_dict, orient='index', columns=['POSITIVE_NEGATION', 'POSITIVE', 'FIFTY_FIFTY_BLOCKED', 'FIFTY_FIFTY_RANDOM', 'NEUTRAL'])
cur_help_table.to_excel('cur_help_table.xlsx')
